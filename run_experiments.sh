...
# Tạo header cho CSV
echo "experiment_id,csv_file,model_type,hidden_size,num_layers,batch_size,learning_rate,epochs,mse,mae,rmse,train_months,test_month" > "$RESULTS_CSV"
            ...
            for num_layers in "${NUM_LAYERS[@]}"; do
                for batch_size in "${BATCH_SIZES[@]}"; do
                    for lr in "${LEARNING_RATES[@]}"; do
                        ...
                        # Sử dụng sed để thay đổi các giá trị trong config
                        # Thay đổi csv_path
                        ...
                        if [ $? -eq 0 ]; then
                            ...
                            TRAIN_MONTHS=$(echo "$OUTPUT" | grep "Train months:" | awk '{print $3}')
                            TEST_MONTH=$(echo "$OUTPUT" | grep "Test month:" | awk '{print $3}')
                            ...
                            # Ghi kết quả vào CSV
                            echo "$EXPERIMENT_ID,$csv_name,$model_type,$hidden_size,$num_layers,$batch_size,$lr,$NUM_EPOCHS,$MSE,$MAE,$RMSE,$TRAIN_MONTHS,$TEST_MONTH" >> "$RESULTS_CSV"
                        else
                            echo "✗ Training thất bại"
                            # Ghi lỗi vào CSV
                            echo "$EXPERIMENT_ID,$csv_name,$model_type,$hidden_size,$num_layers,$batch_size,$lr,$NUM_EPOCHS,ERROR,ERROR,ERROR,N/A,N/A" >> "$RESULTS_CSV"
                        fi
